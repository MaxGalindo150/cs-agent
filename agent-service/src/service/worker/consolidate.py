...
from agent.memory.consolidation import consolidate_all_due_users
...
def _require_worker_token(
    ...
    rather than silently accepting one (docs/SECURITY.md)."""
    if (
        not settings.internal_worker_token
        or x_worker_token != settings.internal_worker_token
    ):
...
async def consolidate(
    ...
    """Sweep up to a batch of due users; returns new facts written per
    user_id (0 for a user swept but not yet due). A backlog larger than the
    batch is left for the next periodic trigger — see agent/memory/
